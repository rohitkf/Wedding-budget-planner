...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!getSubcategory(id)) return jsonError("Subcategory not found", 404);
    return NextResponse.json(listPayments(id));
  } catch (err) {
  ...
  try {
    ...
    for (const split of input.splits) {
      if (split.sourceType === "SAVINGS") {
        if (!split.savingsAccountId || !getSavingsAccount(split.savingsAccountId)) {
          throw new ApiValidationError("Savings account not found for one of the splits");
        }
      } else {
        if (!split.creditCardId || !getCreditCard(split.creditCardId)) {
          throw new ApiValidationError("Credit card not found for one of the splits");
        }
      }
    }

    const payments = recordPayments(id, input.splits.map((s) => ({ ...s, isDeposit: input.isDeposit, paidAt: input.paidAt })));
